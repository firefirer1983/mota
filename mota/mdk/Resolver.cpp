...
void Resolver::onSockCreate(int sockfd, int type)
{
  ...
  channel->setReadCallback(std::bind(&Resolver::onRead, this, sockfd, std::placeholders::_1));
  channel->enableReading();
//  channels_.insert(sockfd, channel);
	auto res = channels_.insert(std::make_pair(sockfd, channel));
  if (res.second == false) {
